...
export function Sidebar({ navigation }: Omit<SidebarProps, 'layout' | 'onLayoutChange'>) {
  ...
  if (isMobile) {
    return (
      <>
        <header className="fixed top-0 left-0 right-0 h-16 bg-[var(--sidebar-bg)] z-40 flex items-center px-4 print:hidden">
          <button
  ...
  if (currentLayout === 'header') {
    return (
      <header className="fixed top-0 left-0 right-0 h-16 bg-[var(--sidebar-bg)] z-40 flex items-center px-5 print:hidden">
  ...
  return (
    ...
    >
      ...
      <a
        href="/"
        className="h-14 flex items-center hover:bg-[var(--sidebar-hover)] transition-colors px-4"
      >
